...
export function usePieData() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      if (ec) throw ec;
      if (ep) throw ep;
      setCursos((c ?? []).map(mapCurso));
      setProfesionales((p ?? []).map(mapProfesional));
    } catch (e: unknown) {
      ...
      .on('postgres_changes',
        ...
        payload => {
          if (payload.eventType === 'INSERT') {
            ...
            setCursos(prev => prev.filter(c => c.id !== (payload.old as Record<string, unknown>).id));
          }
        }
      ).subscribe();
      ...
      .on('postgres_changes',
        ...
        payload => {
          if (payload.eventType === 'INSERT') {
            ...
            setProfesionales(prev => prev.filter(p => p.id !== (payload.old as Record<string, unknown>).id));
          }
        }
      ).subscribe();
